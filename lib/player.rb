...
# Player - find legal piece offsets and captures
class Player
  MAX_DISTANCE = 3
  ...
  def find_pieces(board)
    ...
    3.downto(0).each do |row_num|
      (0..3).each do |col_num|
        ...
        pieces << board.row_col_to_algebraic(row_num, col_num) if square != '_'
      end
    end
  ...
  def find_captures_for_offsets(board, algebraic, offsets)
    ...
    offsets.each_slice(2) do |row_offset, col_offset|
      ...
      if board.valid_board_row_col?(row + row_offset, col + col_offset)
  ...
  def find_offsets_for_direction(board, algebraic, row_inc, col_inc, max_distance)
    ...
    (1..max_distance).each do |distance|
      ...
      if board.valid_board_row_col?(target_row, target_col)
